...
export class SuggestedMealItemDto {
  ...
  @ApiProperty({ example: 'Cung cấp lượng protein tinh khiết từ ức gà và trứng, calo vừa vặn với budget tối.' })
  reason: string;

  @ApiProperty({ example: ['300g phở gà ức', '2 quả trứng chần', 'Nhiều giá đỗ, rau thơm'], required: false })
  ingredients?: string[];
}

export class SuggestMealResponseDto {
  @ApiProperty({ type: () => NutritionGapDto, description: 'Tình trạng dinh dưỡng hiện tại' })
  nutritionGap: NutritionGapDto;

  @ApiProperty({ type: [SuggestedMealItemDto], description: 'Danh sách 1-2 món ăn gợi ý' })
  suggestions: SuggestedMealItemDto[];

  @ApiProperty({ example: 'Hôm nay bạn còn thiếu 45g Protein và 650 kcal. Hãy ưu tiên bữa tối giàu đạm nhé!' })
  advice: string;
}

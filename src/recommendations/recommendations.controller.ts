...
export class RecommendationsController {
  ...
  @ApiOperation({
    summary:
      'Lấy danh mục các nhóm món ăn (Cơm, Bún phở, Thịt gà, Bò, Cá, Rau canh, Trái cây)',
  })
  ...
  @Get('foods')
  @ApiOperation({
    summary: 'Tra cứu danh sách hơn 120+ món ăn Việt Nam chuẩn Calo & Macros',
  })
  @ApiQuery({
    name: 'q',
    required: false,
    description: 'Từ khóa tìm kiếm (VD: phở, gà, bò, cá lóc...)',
  })
  @ApiQuery({
    name: 'category',
    required: false,
    description:
      'Lọc theo nhóm món (VD: "Cơm - Tinh bột", "Bún - Phở - Mì", "Hải sản - Cá"...)',
  })
  ...
  @ApiOperation({
    summary:
      'Gợi ý lộ trình bài tập theo tuần phù hợp với thể trạng BMI & Mục tiêu',
  })
  ...
  @Get('overview')
  @ApiOperation({
    summary: 'Lấy trọn bộ gói gợi ý cá nhân hóa (Thực đơn + Lộ trình bài tập)',
  })
  ...
  @ApiOperation({
    summary:
      'Lấy danh sách 50 bài tập theo Giới tính (MALE / FEMALE) và Cấp độ (BEGINNER / INTERMEDIATE / ADVANCED)',
  })
  ...
  async getGenderExercises(
    ...
    return this.recommendationsService.getGenderExercises(
      userId,
      gender,
      level,
    );
  ...
  @ApiOperation({
    summary:
      'Xem chi tiết hướng dẫn thực hiện, lỗi sai và cách hít thở của 1 bài tập',
  })
  ...
  @ApiOperation({
    summary:
      'Tra cứu thông tin dinh dưỡng theo mã vạch sản phẩm (qua OpenFoodFacts)',
  })
  @ApiResponse({
    status: 200,
